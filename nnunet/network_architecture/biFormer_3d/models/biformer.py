...
sys.path.append("/hy-tmp/nnUNet-nnunetv1/nnunet/network_architecture/biFormer_3d")
from ops.bra_legacy import BiLevelRoutingAttention

from nnunet.network_architecture.biFormer_3d.models._common import Attention, AttentionLePE, DWConv
...
class Block(nn.Module):
    def __init__(self, dim, drop_path=0., layer_scale_init_value=-1,
                       num_heads=8, n_win=7, d_win=8,qk_dim=None, qk_scale=None,
        ...
        # modules
        if before_attn_dwconv > 0: # 用深度可分离卷积编码位置信息
            self.pos_embed = nn.Conv3d(dim, dim,  kernel_size=before_attn_dwconv, padding=1, groups=dim)
        else:
            self.pos_embed = lambda x: 0
        self.norm1 = nn.LayerNorm(dim, eps=1e-6) # important to avoid attention collapsing
        if topk > 0:
            self.attn = BiLevelRoutingAttention(dim=dim, num_heads=num_heads, n_win=n_win, d_win=d_win,qk_dim=qk_dim,
    ...
    def forward(self, x):
        ...
        # attention & mlp
        if self.pre_norm: #先正则再注意力还是先注意力再正则？
...
class BiFormer(nn.Module):
    def __init__(self, depth=[3, 4, 8, 3], in_chans=3, num_classes=1000, embed_dim=[64, 128, 320, 512],
                 ...
                 pe=None,
                 pe_stages=[0],
                 before_attn_dwconv=3, #dwc卷积核的大小
                 auto_pad=False,
                 #-----------------------
        ...
        Args:
            ...
            drop_path_rate (float): stochastic depth rate 随机深度率:让一部分路径失效
            norm_layer (nn.Module): normalization layer
            conv_stem (bool): whether use overlapped patch stem
        """
        super().__init__()
        self.num_classes = num_classes  # 分多少个类别
        self.num_features = self.embed_dim = embed_dim  # num_features for consistency with other models 
        #每个token的维度

        ############ downsample layers (patch embeddings) ######################
        self.downsample_layers = nn.ModuleList() # 1 patch embedding+3 patch merging
        ...
        ) # 第一个stage的patch embedding
        if (pe is not None) and 0 in pe_stages: # 位置编码，不使用 
        ...
        self.stages = nn.ModuleList() # 4 feature resolution stages, each consisting of multiple residual blocks   4个stage中的Biformer block
        nheads= [dim // head_dim for dim in qk_dims] # 每一个stage有多少个头
